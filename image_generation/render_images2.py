...
def main(args):
  ...
  # Create directory for output_scene_file if it doesn't exist
  output_scene_dir = os.path.dirname(args.output_scene_file)
  if output_scene_dir and not os.path.isdir(output_scene_dir):
...
def add_random_objects(scene_struct, num_objects, args, camera):
  ...
  # Check visibility but don't enforce minimum visibility
  visibility_info = check_visibility(blender_objects, args.min_pixels_per_object)

  # Add visibility information to objects
  for i, obj_data in enumerate(objects):
    visible_pixels = visibility_info.get(i, 0)
    obj_data['visible_pixels'] = visible_pixels
    obj_data['visible'] = visible_pixels >= args.min_pixels_per_object

  print(f'Object visibility: {", ".join([f"{i}: {v} px" for i, v in visibility_info.items()])}')
...
def check_visibility(blender_objects, min_pixels_per_object):
  """
  Check how many pixels each object occupies in the rendered image.
  Returns a dictionary mapping object indices to pixel counts.
  """
...
def render_shadeless(blender_objects, path='flat.png'):
  ...
  for i, obj in enumerate(blender_objects):
    ...
    object_colors.append((r, g, b, 1.0))
...
# Add this function to render_images.py
def print_gpu_info():
    """Print information about GPU usage in Blender."""
    if hasattr(bpy.context.scene, 'cycles'):
        cycles = bpy.context.scene.cycles
        print("\n=== GPU RENDERING INFO ===")
        print(f"Device being used: {cycles.device}")
